            ...
            async (position) => {
                try {
                    const response = await fetch(
                        `${API_BASE_URL}/weather?lat=${position.coords.latitude}&lon=${position.coords.longitude}&appid=${API_KEY}&units=metric&lang=ja`
                    );
...
window.addEventListener('load', async () => {
    if (API_KEY === 'YOUR_API_KEY_HERE') {
        showError('APIキーを設定してください。js/app.jsファイルを編集し、OpenWeatherMap APIキーを入力してください。');
        return;
    }
